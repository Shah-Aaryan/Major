...
class CMAESOptimizer(BaseOptimizer):
    ...
    def optimize(self, baseline_objective: Optional[float] = None) -> OptimizationResult:
        ...
        has_cmaes = importlib.util.find_spec("cmaes") is not None

        if has_cmaes and OPTUNA_AVAILABLE:
            sampler_cls = (
        ...
        if has_cmaes:
            try:
                ...
                logger.warning(f"Standalone cmaes failed ({e}), falling back to built-in Gaussian search")

        # --- Path C: Simple adaptive Gaussian search fallback ---
